...
function TopBar({ route, crumbExtra, rightExtra, pendingCount, onBell }) {
  ...
  // Keep our local state in sync if the Tweaks panel flips the theme.
  React.useEffect(() => {
  ...
  const toggleTheme = () => {
    ...
    // Prefer the central tweaks setter so the panel state stays in sync.
    if (typeof window.setTweak === 'function') {
      window.setTweak('theme', next);
    } else {
