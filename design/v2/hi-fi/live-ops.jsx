...
function PipelineCanvas({ paused, intensity, onCounters }) {
  ...
  useELO(() => {
    ...
    const draw = (ts) => {
      ...
      const COL = dark
        ? { laneN: '#1f1d18', laneL2: '#2a2210', laneL3: '#241830',
            stroke: '#2c2924', labelDim: '#aaa599', ink: '#f0ede4',
            warn: '#e6b86b', scrub: '#cfa3eb', info: '#9fbef5',
            infoFill: 'rgba(159, 190, 245, 0.10)', infoStroke: 'rgba(159, 190, 245, 0.55)',
            ringStroke: '#3a3630', ringWarn: '#e6b86b', okFg: '#8ec898' }
        : { laneN: '#fafaf7', laneL2: '#fbf2dc', laneL3: '#f0e3f7',
            stroke: '#d8d4c7', labelDim: '#5a5a5a', ink: '#1a1a1a',
            warn: '#8a5a00', scrub: '#5a1a8a', info: '#1d3a7a',
            infoFill: 'rgba(29, 58, 122, 0.10)', infoStroke: 'rgba(29, 58, 122, 0.55)',
            ringStroke: '#d8d4c7', ringWarn: '#8a5a00', okFg: '#22592a' };
      ...
      // lane backgrounds
      const laneCols = {
        agents: { x: 0.04 * w,  w: 0.10 * w, fill: COL.laneN },
        l1:     { x: 0.22 * w,  w: 0.13 * w, fill: COL.laneN },
        l2:     { x: 0.43 * w,  w: 0.13 * w, fill: COL.laneL2 },
        l3:     { x: 0.64 * w,  w: 0.13 * w, fill: COL.laneL3 },
        ext:    { x: 0.85 * w,  w: 0.10 * w, fill: COL.laneN },
      };
      ...
      if (stuckL2.length > 0) {
        ...
        const cy = h * 0.55;
        ctx.fillStyle = COL.infoFill;
        ctx.strokeStyle = COL.infoStroke;
        ctx.lineWidth = 1.5;
        ctx.setLineDash([3, 2]);
        ctx.beginPath();
...
function CastleMoat({ paused, intensity }) {
  ...
  useELO(() => {
    ...
    const draw = (ts) => {
      ...
      const dark = document.documentElement.dataset.theme === 'dark';
      const COL = dark
        ? { r1: '#1f1d18', r2: '#26231d', r3: '#2c2823', rC: '#2a2210',
            stroke: '#3a3630', strokeWarn: '#e6b86b', labelDim: '#aaa599',
            warn: '#e6b86b', ink: '#f0ede4' }
        : { r1: '#fafaf7', r2: '#f4f2eb', r3: '#ebe9e2', rC: '#fbeed1',
            stroke: '#d8d4c7', strokeWarn: '#8a5a00', labelDim: '#5a5a5a',
            warn: '#8a5a00', ink: '#1a1a1a' };
      const rings = [
        { r: Math.min(w, h) * 0.42, fill: COL.r1, label: 'L1 · IDENTITY' },
        { r: Math.min(w, h) * 0.32, fill: COL.r2, label: 'L2 · CAPABILITY' },
        { r: Math.min(w, h) * 0.22, fill: COL.r3, label: 'L3 · SCRUB' },
        { r: Math.min(w, h) * 0.12, fill: COL.rC, label: 'CROWN' },
      ];
      ...
      rings.forEach((ring, i) => {
        ...
        ctx.fill();
        ctx.strokeStyle = i === 3 ? COL.strokeWarn : COL.stroke;
