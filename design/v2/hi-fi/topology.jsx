...
// Edge colors: in dark mode we want brighter strokes against the dark canvas.
// We read the current theme at render time so this re-evaluates on toggle.
const TOPO_EC_LIGHT = {
...
const TOPO_EC_DARK = {
  delegates_to: { color: '#c8c5b8', dash: null,  w: 2,   label: 'delegates_to' },
  calls:        { color: '#9fbef5', dash: '7 3', w: 1.5, label: 'calls'        },
  reads:        { color: '#8ec898', dash: '3 4', w: 1.5, label: 'reads'        },
  writes:       { color: '#e6b86b', dash: '3 4', w: 1.5, label: 'writes'       },
  approves:     { color: '#cfa3eb', dash: '8 3', w: 1.5, label: 'approves'     },
  messages:     { color: '#8a8880', dash: '2 5', w: 1,   label: 'messages'     },
};
const getTopoEC = () => (document.documentElement.dataset.theme === 'dark' ? TOPO_EC_DARK : TOPO_EC_LIGHT);
// Proxy that forwards key/value iteration so Object.entries / Object.keys work.
const TOPO_EC = new Proxy({}, {
  get:    (_, k) => getTopoEC()[k],
  has:    (_, k) => k in getTopoEC(),
  ownKeys: ()    => Reflect.ownKeys(getTopoEC()),
    ...
    <div className="scrim scrim-center" onClick={onClose} style={{ zIndex: 80 }}>
      <div className="modal" style={{ maxWidth: 540 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ padding: 16 }}>
            ...
            return (
              <div key={nd.id} style={{ background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: 3, padding: '10px 12px', marginBottom: 8 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
                  ...
                  <div style={{ background: 'var(--warn-bg)', border: '1px solid var(--warn)', borderRadius: 2, padding: '6px 8px' }}>
          ...
          {/* Backend API preview */}
          <div style={{ marginTop: 12, background: 'var(--code-bg)', borderRadius: 3, padding: '10px 12px', fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--code-fg-soft)', lineHeight: 1.7, whiteSpace: 'pre' }}>
            <span style={{ color: 'var(--code-fg-dim)' }}>{'POST /api/v1/policies/cascade\n'}</span>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <svg ref={svgRef} width="100%" height="100%"
        style={{ cursor: dragging ? 'grabbing' : 'grab', userSelect: 'none', background: 'var(--paper)' }}
        ...
        {[['＋', () => setZoom(z => Math.min(2.5, +(z * 1.2).toFixed(2)))],
          ...
          <button key={lbl} onClick={fn}
            style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: 'JetBrains Mono', fontSize: 13, border: '1px solid var(--line)',
              background: 'var(--paper-2)', borderRadius: 3, cursor: 'pointer', color: 'var(--ink-3)' }}>
            {lbl}
          </button>
        ))}
        <div style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--ink-5)', textAlign: 'center', marginTop: 2 }}>
          {Math.round(zoom * 100)}%
        </div>
      </div>
    </div>
    ...
    <>
      ...
      <div style={{ width: 276, background: 'var(--paper-2)', borderLeft: '1px solid var(--line)', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 14px 16px' }}>
          {/* Cycle alert */}
          {inCycle && (
            <div style={{ marginTop: 10, background: 'var(--danger-bg)', border: '1px solid var(--danger)', borderRadius: 3, padding: '7px 10px', fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--danger)' }}>
...
function TopoSidebar({ filterTeam, setFilterTeam, visEdges, setVisEdges, showCross, setShowCross, liveMode, setLiveMode, hasCycles, liveMsg }) {
  ...
  return (
    <div style={{ width: 192, borderRight: '1px solid var(--line)', background: 'var(--paper-2)', display: 'flex', flexDirection: 'column', overflow: 'auto', flexShrink: 0 }}>
      ...
      {hasCycles && (
        <div style={{ borderTop: '1px solid var(--line)', padding: '11px 13px 8px' }}>
          <div style={{ background: 'var(--danger-bg)', border: '1px solid var(--danger)', borderRadius: 3, padding: '7px 9px', fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--danger)' }}>
