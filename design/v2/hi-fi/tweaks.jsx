...
function applyVisualTweaks(t) {
  ...
  // Theme — swap the entire token palette via data-theme on <html>.
  // We clear the inline --ink override in dark mode so the dark token wins.
  if (t.theme === 'dark') {
    root.dataset.theme = 'dark';
    // Accent colors are tuned for light mode; in dark mode the off-white --ink
    // is intentional, so undo the inline accent override.
...
function AssemblyTweaksPanel() {
  ...
  }, [t.posturePreset, t.density, t.accent, t.theme, t.showFlags, t.streamTheme,
  ...
  window.TWEAKS = t;
  // Expose the setter so the topbar theme toggle (and any other shell chrome)
  // can flip a single tweak without re-rolling its own state.
