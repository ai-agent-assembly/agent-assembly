...
def _channel_map(doc):
    """{channel-id: target} for the channels in a computed versions doc."""
    return {c["id"]: c["target"] for c in doc["channels"]}


def _archived_ids(doc):
    return [a["id"] for a in doc["archived"]]


def _prior(stable=None, pre_release=None, archived=()):
    """Build a prior live manifest seeding the given standing channels."""
    channels = [{"id": "latest", "title": "latest (master)", "target": "latest"}]
    if pre_release is not None:
        channels.append(
            {"id": "pre-release", "title": "pre-release (%s)" % pre_release, "target": pre_release}
        )
    if stable is not None:
        channels.append({"id": "stable", "title": "stable (%s)" % stable, "target": stable})
    return {
        "channels": channels,
...
class ChannelGateScenarioTests(unittest.TestCase):
    def test_scenario_1_pre_release_leads_stable(self):
        ...
        self.assertEqual([c["id"] for c in doc["channels"]], ["latest", "pre-release", "stable"])

    def test_scenario_2_stable_supersedes_pre_release(self):
    ...
    def test_scenario_3_new_pre_release_line_leads_again(self):
    ...
    def test_no_stable_shows_any_pre_release(self):
