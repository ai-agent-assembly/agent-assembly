...
| Network-egress block | **L2** | Corrected by the AAASM-5856 security review: `generate_managed_settings`/`apply_settings` (`aa-devtool-codex/src/lib.rs:219-279`) computes `allowed_domains`/`blocked_domains`, but that step (`StepAction::ApplyLegacyManagedSettings`) has never been reachable through `FilesystemExecutor` — `aa-core/src/integration/engine.rs`'s `StepExecutor::apply` reports `Unsupported` for it — so this was never the live mechanism on the install path, before or after AAASM-5856's native `CodexIntegration`. The native lifecycle (`aa-devtool-codex/src/lifecycle.rs`) writes only `sandbox_mode`/`approval_policy`; it deliberately does not write `allowed_domains`/`blocked_domains`, since `IntegrationRequest` carries policy only by reference (agent-assembly ADR 0030 §5.5) and writing an empty list would silently erase a real one. Network-egress enforcement for Codex therefore rests entirely on the `aa-proxy` leg, which works as of AAASM-5856: the install materialises a copy of the Agent Assembly proxy CA and injects `CODEX_CA_CERTIFICATE` into the launch environment, so the MitM handshake succeeds. |
...
- Network-egress block is `aa-proxy`-only, not sandbox-native — see the row above. The `aa-proxy` leg works as of AAASM-5856: the native `CodexIntegration`'s install plan materialises a copy of the Agent Assembly proxy CA and injects `CODEX_CA_CERTIFICATE` — Codex's own custom-CA mechanism (`codex-rs/http-client/src/custom_ca.rs`), additive to the platform's built-in trust roots — so the MitM handshake succeeds and prompt/response redaction and proxy-observed budget enforcement now have coverage there. Limits: `llm_only` defaults to `true`, so only the built-in LLM hosts are decrypted; and the evidence behind this claim is `aa-integration-tests/tests/cli_run_codex_launch_env.rs`'s stand-in client (`examples/codex_tls_client.rs`, which re-implements Codex's documented CA precedence), not the shipped `codex` binary — a real-binary-gated lane is separate follow-up work. This was fixable, unlike Windsurf below, because the mechanism is native to Codex's own reqwest/rustls stack rather than something AASM has to bolt on.
