...
| Network-egress block | **L2** | Corrected by the AAASM-5856 security review: `generate_managed_settings`/`apply_settings` (`aa-devtool-codex/src/lib.rs:219-279`) computes `allowed_domains`/`blocked_domains`, but that step (`StepAction::ApplyLegacyManagedSettings`) has never been reachable through `FilesystemExecutor` — `aa-core/src/integration/engine.rs`'s `StepExecutor::apply` reports `Unsupported` for it — so this was never the live mechanism on the install path, before or after AAASM-5856's native `CodexIntegration`. The native lifecycle (`aa-devtool-codex/src/lifecycle.rs`) writes only `sandbox_mode`/`approval_policy`; it deliberately does not write `allowed_domains`/`blocked_domains`, since `IntegrationRequest` carries policy only by reference (ADR 0030 §5.5) and writing an empty list would silently erase a real one. Network-egress enforcement for Codex therefore rests entirely on the `aa-proxy` leg, which works as of AAASM-5856: the install materialises a copy of the Agent Assembly proxy CA and injects `CODEX_CA_CERTIFICATE` into the launch environment, so the MitM handshake succeeds. |
