...
| Network-egress block | **L3** | `generate_managed_settings`/`apply_settings` (`aa-devtool-codex/src/lib.rs:219-279`) computes `allowed_domains`/`blocked_domains` (via the `network_allow_list`/`network_block_list` helper functions in `sandbox.rs`) and writes them into the same JSON blob as `sandbox_mode`/`approval_policy`, atomically, to `~/.codex/config.json`. The `aa-proxy` leg of this row now works too, since AAASM-5856: the install materialises a copy of the Agent Assembly proxy CA and injects `CODEX_CA_CERTIFICATE` into the launch environment, so the MitM handshake succeeds. This row's L3 still rests on the sandbox-native write, not on the proxy leg — but the proxy leg is now genuine belt-and-suspenders coverage rather than an absent one. |
...
- The `aa-proxy` leg of network-egress block works as of AAASM-5856: the native `CodexIntegration`'s install plan materialises a copy of the Agent Assembly proxy CA and injects `CODEX_CA_CERTIFICATE` — Codex's own custom-CA mechanism (`codex-rs/http-client/src/custom_ca.rs`), additive to the platform's built-in trust roots — so the MitM handshake succeeds and prompt/response redaction and proxy-observed budget enforcement (both proxy-only, no sandbox-native equivalent) now have coverage there. Two limits remain: `llm_only` defaults to `true`, so only the built-in LLM hosts are decrypted; and the evidence behind this claim is `aa-integration-tests/tests/cli_run_codex_launch_env.rs`'s stand-in client (`examples/codex_tls_client.rs`, which re-implements Codex's documented CA precedence), not the shipped `codex` binary — a real-binary-gated lane is separate follow-up work. This was fixable, unlike Windsurf below, because the mechanism is native to Codex's own reqwest/rustls stack rather than something AASM has to bolt on.
