...
| --- | --- | --- |
| `aa-proxy`, **MCP `tools/call` on a non-LLM MitM'd host, gateway configured** | Yes, synchronously, before dialling upstream | `handle_non_llm_mitm` (`aa-proxy/src/proxy/mod.rs`) calls `evaluate_mcp_request` — *"MCP detection — only when a gateway is configured"*. On `McpEvalOutcome::Deny` the handler answers the client and returns **without** reaching `dial_upstream_tls`. This is one of two gateway-bound pre-dial blocks in the system, alongside the CONNECT-time-egress row below (AAASM-5851). Line numbers deliberately omitted in this row — a prior version of it cited exact line numbers, and AAASM-5851's own diff shifted every one of them, which is the kind of rot function-name-only citations don't suffer. |
| `aa-proxy`, **LLM-provider hosts, payload refusal** (the only hosts MitM'd under the `llm_only` default) | **No** | `handle_llm_mitm`'s payload-level refusal — the `Interceptor`'s `VerdictDecision::Block` (credential/DLP) — is local-only regardless of gateway configuration; this row is unchanged by AAASM-5851. |
| `aa-proxy`, **LLM-provider hosts, in-tunnel `Host`-header re-check** (AAASM-4829 defense) | Same as the two CONNECT-time-egress rows below — gateway-bound when configured, local otherwise | `handle_llm_mitm` also calls `in_tunnel_deny_reason` before the payload path, which (AAASM-5851) now routes through `egress_deny_reason` exactly like the CONNECT-time and plain-HTTP checks — a forged `Host:` header inside an already-open LLM tunnel is subject to the same gateway policy as the original CONNECT, not just the local allowlist. |
...
| **macOS** | `aa-proxy`; a System Keychain trust install **attempted at proxy start unless the launch opts out**, gated on whether the certificate is already installed *and* on `AA_PROXY_SYSTEM_TRUST_INSTALL` (AAASM-5978) — `pub async fn run` (`aa-proxy/src/lib.rs:50`) matches `config::system_trust_install_from_env()?` on macOS (`aa-proxy/src/lib.rs:80-91`): `Never` skips the block entirely, including the `is_installed()` lookup itself, making zero `security` CLI calls; `Auto` (the default — unset reproduces the historical unconditional behaviour) runs `if !ca.is_installed()? { … ca.install()?; }` exactly as before. `CaStore::install` (`aa-proxy/src/tls/ca.rs:374`, calling `keychain::add_trusted_cert` at `:378`) shells out to `security add-trusted-cert` (`fn add_trusted_cert`, `aa-proxy/src/tls/keychain.rs:18`; the `Command::new("security")` invocation is `:23-32`), which **requires admin authorization** — macOS prompts for it (`aa-proxy/src/tls/keychain.rs:16`) — and because `ca.install()?` propagates out of `run`, **a refused prompt fails proxy startup** on the `Auto` path. The Claude Code integration sets `Never`: `aasm run claude` (`aa-cli/src/commands/run.rs`) asks the adapter's `process_scoped_ca_trust_var()` (`aa-core/src/dev_tool.rs`, overridden in `aa-devtool-claude-code/src/lib.rs`) whether this specific launch already has process-scoped trust — it does, established per-launch through `NODE_EXTRA_CA_CERTS` (`aa-devtool-claude-code/src/lifecycle.rs`) — so this integration's managed launches make no System Keychain call at all, not merely "do not rely on" one. Standalone `aasm proxy start` is unaffected and keeps `Auto`. Regression coverage: `aa-integration-tests/tests/proxy_system_trust_install.rs` (real spawned binary, `security` invocation recorder) and `aa-proxy/tests/system_trust_install_does_not_weaken_validation.rs` (proves the skip never weakens certificate validation). | **None.** Endpoint Security / Network Extension is an **explicit non-goal** — asserted in product docs (`docs/src/devtools/product-brief.md:448,655` — *"macOS Endpoint Security and Network Extension remain explicit non-goals"*, and `aa-ebpf` is *"Linux-only and is a **detection** layer that cannot modify traffic in flight"*) and pinned by a test asserting the literal limitation string (`aa-cli/src/commands/integrations/model.rs:1200,1204`) | Transport mediation **Implemented**; **E4 host-level interception Unsupported**. **Do not read this as "no host enforcement on macOS"** — see the note below: macOS is the *only* platform on which ADR 0030's `HostEnforced` rung is reachable today. |
...
banned absolutes bind *architecture and product descriptions*, and **may not be
waived** by anyone, for any period —
[ADR 0034 Decision 10](0034-one-product-truth-and-cross-repository-documentation-governance.md#10-waivers-and-exceptions)
settled that question, which was 5621's too (AAASM-5671). How the ban is policed
...
| [AAASM-5978](https://lightning-dust-mite.atlassian.net/browse/AAASM-5978) | Makes §5.3's macOS CA install conditional — `AA_PROXY_SYSTEM_TRUST_INSTALL=never` skips it for a managed launch with process-scoped trust already established |
