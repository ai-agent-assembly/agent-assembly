...
| **6.4 Core runtime distribution and update** | `aa-runtime` + `aa-gateway` + `aa-proxy` + the linked adapters | **One versioned unit**, owned by the AASM release process (Homebrew tap, container image, installer). Version is reported over the DI-API `HelloAck` | Trusted. Its updates are never triggered silently by a thin client |
...
| [AAASM-5982](https://lightning-dust-mite.atlassian.net/browse/AAASM-5982) | §6.4: adds `aa-proxy` to the one-versioned-unit list, matching row 7's placement of model-path protection in L-D core; aligns `aa-proxy`'s runtime resolution with the exe-sibling-first search this row already requires |
