...
|---|---|---|---|
| **Bounded region** | `<!-- BEGIN GENERATED… -->` … `<!-- END GENERATED… -->`, in **three spellings** — see below | this repo's `scripts/check_contact_metadata.py`; Docs Hub `generate_hub_components.py` and `generate_compatibility.py` | Part of a hand-written file |
...
| **Unmarked stamped literal** | *none* | `scripts/propagate_versions.py` | Individual values inside otherwise hand-written prose |
