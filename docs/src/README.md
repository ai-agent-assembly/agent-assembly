...
New here? Start with the **[Introduction](introduction/README.md)** — it explains what Agent Assembly is, the problem it solves, the core concepts, and the three enforcement mechanisms. Then move on to the [Quick Start](quick-start/requirements.md).
...
From there, attach an SDK shim, the `aa-proxy` sidecar, or eBPF to start observing agent actions. The [Architecture](architecture/README.md) chapter explains how those three mechanisms fit together.
