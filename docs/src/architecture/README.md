...
that governs AI agents by observing their actions through three
independently-deployable mechanisms and routing the governed actions through
one central **gateway**.
...
- **[System architecture](system-architecture.md)** — the big picture: the 28
  workspace crates, the three enforcement mechanisms, the gateway / API / runtime /
...
- **[Component deep-dives](components.md)** — a per-crate tour of responsibilities,
  ...
  three enforcement-mechanism crates, API, CLI, foundation crates, storage, and cache.
...
```mermaid
flowchart LR
    Agent[AI agent] --> Mechs["3 independently-deployable mechanisms<br/>SDK · proxy · eBPF · no combined guarantee"]
    Mechs --> RT["aa-runtime<br/>chokepoint"]
