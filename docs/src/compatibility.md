...
| AAASM-2338 | Added `publish = false` to workspace `[workspace.package]` + removed `Publish to crates.io` job from `release.yml`. `aa-cli` is no longer distributed via crates.io; install via Homebrew tap, GHCR Docker images, or GitHub Releases binaries. Resolves AAASM-2094 (cargo publish dep resolution gap). | None — distribution channel removal only; no public API, protocol, or ABI changes. `cargo install aasm` stops working but no other behavior shifts. |
