...
| AAASM-2849 | Bumped workspace + 43 path-dep version literals (35 historical + 8 storage/cache from AAASM-2797) from `0.0.1-alpha.8` to `0.0.1-alpha.9`. Ninth pre-release in the v0.0.1 dry-run series. Unlike alpha-5 through alpha-8, this bump is **not** a recovery from a publish-crates blocker — alpha-8 completed end-to-end. alpha-9 carries agent-assembly docs polish forward (AAASM-2199 examples link, AAASM-2827 archive retention, AAASM-2833 release badge, AAASM-2841 version-selector typography) and drives a fresh fan-out so SDK content merged since alpha-8 lands downstream. Particularly drives `@agent-assembly/sdk@0.0.1-alpha.9` to npm carrying AAASM-2842 (public `GatewayClient` + `createNoopGatewayClient` re-exports) and AAASM-2829 (node-sdk docs archive backfill). On crates.io, all 14 publishable crates re-publish at `0.0.1-alpha.9` alongside their existing `0.0.1-alpha.8` rows. | None — pre-release version bump; AAASM-2340 behaviour delta (held-back `aasm run` / `aasm tools` on crates.io) carries forward unchanged. |
