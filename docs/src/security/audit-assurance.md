...
| **Gateway SQL** — `audit_events` (`aa-gateway/migrations/postgres/0001_initial.sql`) | The dual-sink copy of the decision record. The runtime-to-storage conversion deliberately drops `seq`, `previous_hash` and `entry_hash` — this table has no column for any of the three — so it is not the tamper-evident tier and `aasm audit verify-chain` cannot run against it | Time: `hot_days` + `warm_days`, then `cold_action` | `apply_retention` issues `DELETE FROM audit_events` (`aa-gateway/src/storage/postgres.rs`, `sqlite.rs`) |
| **`audit_logs`** (`aa-storage-postgres/migrations/0004_audit_logs.sql`) | The metadata-only row written by the NATS audit consumer (`aa-storage-postgres/src/audit_sink.rs`). Same gap as `audit_events`: no `seq`/`previous_hash`/`entry_hash` columns, so this table is not chain-verifiable either | **None** | **Nothing.** No retention pass in this repository targets this table |
