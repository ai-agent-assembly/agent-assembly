//! AAASM-5984 AC2/AC6: `aa-gateway` states its build identity in `--version`
...
//!
//! `env!("CARGO_BIN_EXE_aa-gateway")` makes cargo build the binary as a
...
fn version_output() -> String {
    let out = Command::new(env!("CARGO_BIN_EXE_aa-gateway"))
        .arg("--version")
        .output()
        .expect("run aa-gateway --version");
    assert!(out.status.success(), "aa-gateway --version should exit 0");
...
fn a_version_only_assertion_cannot_distinguish_a_foreign_build() {
    ...
    // The identity assertion can — this is the whole point of AC2/AC3.
    assert_ne!(
...
/// AC2: the startup log line names the build. Default `legacy-grpc` mode with
/// no `--policy` fails fast (`"--policy is required in legacy-grpc mode"`)
/// right after the identity line and before storage/network — deterministic
/// and cheap.
#[test]
fn startup_log_states_the_build_identity() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_aa-gateway"))
        .env("RUST_LOG", "info")
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            let _ = child.kill();
            panic!("aa-gateway did not exit within 10s with no --policy in legacy-grpc mode");
        }
        std::thread::sleep(Duration::from_millis(20));
    };
    assert!(!status.success(), "legacy-grpc mode with no --policy should fail fast");
    ...
    assert!(
        combined.contains(BUILD_SHA) && combined.contains("aa-gateway starting"),
        "startup log should state the build identity; got:\n{combined}"
    );
}

/// AC6, structural: a `BuildIdentity::of_this_build()` call made from this
/// integration-test binary must equal the one the running `aa-gateway`
/// binary reports — same compiled constants, same `cargo build`.
#[test]
