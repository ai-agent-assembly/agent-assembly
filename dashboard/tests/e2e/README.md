...
**What the gate does not prove:** all 44 gated specs stub every network call (43 via `page.route`, `violations-heatmap` via the shared `mockApi()` helper), so it compares the frontend against its own mocks — it is not an API-contract check and cannot see backend drift. Real-backend coverage is exactly zero: `hitl-approval` is the only spec in the suite that talks to a live gateway, and it needs `cargo`, which this job has no toolchain for. See ADR-0028 for the full list of non-claims.
