 ...
 *
 * Still exported for the nine modules recorded in
 * `__tests__/foldAudit.test.ts` (AAASM-5380 migrated the two approvals surfaces
 * off it). Exactly **one** of those modules is recorded safe — `AppShell`, whose
 * alerts query throws in `parseAlertList` before any fold sees the body. The
 * other eight are live defects. (Several individual *folds* inside those eight
 * are safe; the modules are not, and an earlier draft of this note blurred the
 * two.) The deprecation is a signpost for new code; the enforcement is the
 * `no-restricted-imports` allowlist in `.eslintrc.cjs`. AAASM-5380 is the
 * umbrella ticket retiring the rest.
 */
