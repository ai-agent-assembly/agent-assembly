...
describe('LoginPage', () => {
  it('disables the submit button until an api key is entered', () => {
    renderLogin(vi.fn<(apiKey: string) => Promise<void>>())
  ...
  it('logs in with the trimmed key and navigates home on success', async () => {
    const login = vi.fn<(apiKey: string) => Promise<void>>().mockResolvedValue(undefined)
    renderLogin(login)
    fireEvent.change(screen.getByLabelText('API Key'), { target: { value: '  aa_key  ' } })
  ...
  it('shows an error message and stays on the page when login fails', async () => {
    const login = vi.fn<(apiKey: string) => Promise<void>>().mockRejectedValue(new Error('Authentication failed (401)'))
    renderLogin(login)
