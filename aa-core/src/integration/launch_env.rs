...
//!
//! AAASM-5276 condition C1 is that a tool-trusted CA variable has to point at
//! the proxy CA for interception to work at all. A launcher that simply called
...
//! [`StepAction::InjectLaunchEnvironment`](super::StepAction) whose artifact
//! behaves like every other one in the plan.
//!
...
//! including via `..`.
//!
//! # Why this lives in `aa-core` (AAASM-5914)
//!
//! Originally `aa-devtool-claude-code`-only (AAASM-5276). Nothing about
//! [`LaunchEnvStore`] is Claude-specific — every tool whose plan injects a
//! launch-environment variable (Codex's `CODEX_CA_CERTIFICATE`, and any future
//! adapter) needs the identical mechanism. Promoting it here, re-exported by
//! `aa-devtool-contract`, keeps one implementation for every adapter rather than
//! a second copy diverging in the next crate that needs it.
...
/// every scope directory `dirs` names.
///
/// `dirs` is iterated in order, and a later directory's variables win over an
/// earlier one's for the same name — the caller supplies the precedence (e.g.
/// user scope first, project scope second, so a project-scoped install wins
/// for a variable both set: the narrower installation is the more specific
/// answer for the directory the launch is happening in).
///
...
pub fn installed_environment(dirs: impl IntoIterator<Item = PathBuf>) -> BTreeMap<String, String> {
...
mod tests {
    ...
    #[test]
    fn a_later_directory_wins_over_an_earlier_one_for_the_same_variable() {
        let dir = tempfile::tempdir().unwrap();
        let user_dir = dir.path().join("user");
        let project_dir = dir.path().join("project");

        LaunchEnvStore::at(&user_dir)
            .set("NODE_EXTRA_CA_CERTS", "/user/ca.pem")
            .unwrap();
        LaunchEnvStore::at(&user_dir)
            .set("HTTPS_PROXY", "http://127.0.0.1:8899")
            .unwrap();
        LaunchEnvStore::at(&project_dir)
            .set("NODE_EXTRA_CA_CERTS", "/project/ca.pem")
            .unwrap();

        let env = installed_environment([user_dir, project_dir]);
        assert_eq!(
    ...
    #[test]
    fn no_directories_injects_nothing() {
        assert!(installed_environment(std::iter::empty()).is_empty());
    }
}
