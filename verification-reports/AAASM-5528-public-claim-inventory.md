...
| A38 | `.claude/CLAUDE.md`, `security/three-layer-defense.md`, `governance/capability-matrix.md`, `quick-start/requirements.md` (branch positions) | "CA trust-store install is automatic at proxy start on macOS" | platform-overreach | qualify | Replaced with the attempted/prompted/fails-startup formulation. **This was the same defect as A-row B2, committed inside the fix for it** — a platform claim generalised from a `cfg(target_os)` guard without reading what the guarded call requires. | E2 |
