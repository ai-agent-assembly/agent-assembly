...
mod tests {
    ...
    /// A hash minted by the previous argon2 release must still verify (AAASM-6027).
    ///
    /// Stored password hashes outlive the library that wrote them, so an argon2
    /// upgrade that could not read them would lock every existing native account
    /// out — a failure no other test here would catch, because they all mint and
    /// verify with the *same* build. The vectors below were generated by argon2
    /// 0.5.3 at the ADR 0031 §Q2 parameters and are frozen deliberately: their
    /// value is precisely that they were not produced by the current code.
    ///
    /// PHC is a versioned interchange format, so this is expected to hold across
    /// upgrades — this test is what turns that expectation into a checked one.
    #[test]
    fn hashes_minted_by_the_previous_argon2_release_still_verify() {
        // (plaintext, PHC hash written by argon2 0.5.3). Throwaway strings, never
        // real credentials — the hash is the fixture, the plaintext only exists to
        // check it.
        let vectors: &[(&str, &str)] = &[
            (
                "correct-horse-battery",
                "$argon2id$v=19$m=19456,t=2,p=1$HM68+M41xmk8F2lmzLCKuw$r7kKtovjFkcZatVzK6FVUNKHEJcT4XJHIac2yNOp3CM",
            ),
            // Single-character password: exercises the short-input edge.
            (
                "a",
                "$argon2id$v=19$m=19456,t=2,p=1$thcoPP/ylf/PiQ5vXQcDSQ$YB7BELqy6lqX8d5U06+1MRnIgHJ/oqYTEtPt07snV3g",
            ),
        ];

        for (plaintext, stored) in vectors {
            assert!(
                verify_password(stored, plaintext),
                "a hash minted by the previous argon2 release must still verify: {stored}"
            );
            // And the old hash must not have become permissive in the process.
            assert!(
                !verify_password(stored, &throwaway_password()),
                "a wrong password must not verify against an old hash: {stored}"
            );
        }
    }
}
